...
def parse_bc(barcodes, fc, ln):
    ...
    file_in = open(barcodes, 'rU')
    bc_dict = {}
    for line in file_in.readlines():
        if line.startswith("#") or line.startswith("Flowcell"):
			print "Line starts with Flowcell"
			continue
        try:
...
def parse_seq_pe(opts, bc_dict, Flowcell, Lane):
    ...
    if opts.reads1.endswith('.gz'):
        ...
        try:
            ...
            seq2_handle = open(opts.reads2, "rb")
        except IOError:
    ...
    if not opts.split:
        ...
        if opts.reads1.endswith('.gz'):
            ...
            seq1_out = open(os.path.join(opts.output, seq1_name), 'wb')
            seq2_out = open(os.path.join(opts.output, seq2_name), 'wb')
    if opts.reads1.endswith('.gz'):
        nomatch1_out= gzip.open(opts.nomatch1,  "wb")
        nomatch2_out= gzip.open(opts.nomatch2, "wb")
    else:
        nomatch1_out= open(opts.nomatch1,  "wb")
        nomatch2_out= open(opts.nomatch2, "wb")
    seq = 0
...
def write_stats(bc_dict, opts):
    ...
    barcode_in = open(opts.barcode, 'rU')
